    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div class="flex items-center gap-4">
        ...
        <div class="flex items-center gap-2">
          ...
          @php $tidA = $A['team_id'] ?? null; @endphp

            <a href="{{ route('nba.team.show', ['team' => $tidA]) }}" class="text-white font-semibold hover:text-[#84CC16]">
              {{ $A['team'] ?? '—' }}
            </a>
        ...
        {{-- Komanda B --}}
        <div class="flex items-center gap-2">
          @php $tidB = $B['team_id'] ?? null; @endphp

            <a href="{{ route('nba.team.show', ['team' => $tidB]) }}" class="text-white font-semibold text-right hover:text-[#84CC16]">
              {{ $B['team'] ?? '—' }}
            </a>
    ...
    <section class="bg-[#1f2937] border border-[#374151] rounded-xl overflow-hidden">
      <div class="px-4 py-3 bg-[#0f172a] border-b border-[#374151] flex items-center justify-between">
        <h2 class="font-semibold">
            ...
            <a href="{{ route('nba.team.show', ['team' => $tidA]) }}" class="hover:text-[#84CC16]">{{ $A['team'] ?? 'Komanda A' }}</a>
      ...
      <div class="overflow-x-auto">
        <table class="min-w-[900px] w-full text-sm js-sortable-table">
          <thead class="bg-[#0f172a] text-gray-300">
            <tr>
              <th class="px-3 py-2 text-left  sortable" data-type="text">Spēlētājs <span class="arrow">↕</span></th>
              <th class="px-3 py-2 text-center sortable" data-type="time">MIN <span class="arrow">↕</span></th>
              <th class="px-3 py-2 text-center sortable" data-type="ma">Metieni <span class="arrow">↕</span></th>
              <th class="px-3 py-2 text-center sortable" data-type="ma">3P <span class="arrow">↕</span></th>
              <th class="px-3 py-2 text-center sortable" data-type="ma">Sodi <span class="arrow">↕</span></th>
              <th class="px-3 py-2 text-center sortable" data-type="num">Atl.b. <span class="arrow">↕</span></th>
              <th class="px-3 py-2 text-center sortable" data-type="num">Rez.p. <span class="arrow">↕</span></th>
              <th class="px-3 py-2 text-center sortable" data-type="num">Pārtv. <span class="arrow">↕</span></th>
              <th class="px-3 py-2 text-center sortable" data-type="num">Bloki <span class="arrow">↕</span></th>
              <th class="px-3 py-2 text-center sortable" data-type="num">Kļ. <span class="arrow">↕</span></th>
              <th class="px-3 py-2 text-center sortable" data-type="num">Piez. <span class="arrow">↕</span></th>
              <th class="px-3 py-2 text-right  sortable" data-type="num">Punkti <span class="arrow">↕</span></th>
            </tr>
          </thead>
          <tbody class="divide-y divide-[#374151] text-[#F3F4F6]">
            @foreach($A['players'] as $p)
              ...
              <tr class="odd:bg-[#1f2937] even:bg-[#111827]">
                <td class="px-3 py-2" data-player="{{ \Illuminate\Support\Str::lower($pName) }}">
                  <div class="flex items-center gap-2">
                    @if($pImg)
                    ...
                    @endif
                      ...
                      <a class="whitespace-nowrap hover:text-[#84CC16]" href="{{ route('nba.player.show', ['player' => $pid]) }}">
                        {{ $pName }}
                      </a>
    ...
    <section class="bg-[#1f2937] border border-[#374151] rounded-xl overflow-hidden">
      <div class="px-4 py-3 bg-[#0f172a] border-b border-[#374151] flex items-center justify-between">
        <h2 class="font-semibold">
            ...
            <a href="{{ route('nba.team.show', ['team' => $tidB]) }}" class="hover:text-[#84CC16]">{{ $B['team'] ?? 'Komanda B' }}</a>
    ...
    headers.forEach((th) => {
      ...
      const toggle = () => {
        ...
        const rows = Array.from(tbody.querySelectorAll('tr'))
          .filter(tr => !tr.classList.contains('bg-[#0b1220]')); // kopsavilkuma rindu nelokam
